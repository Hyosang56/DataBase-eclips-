package ch13_classes.ex07.service;

import ch13_classes.ex07.dto.BoardDTO;
import ch13_classes.ex07.dto.CommentDTO;
import ch13_classes.ex07.repository.BoardRepository;
import ch13_classes.ex07.repository.CommentRepository;
...
public class BoardService {
    ...
    public void findAll() {
        Map<Long,BoardDTO> boardDTOMap= boardRepository.findAll();
        System.out.println("id\t title\t writer\t hits\t date\t");
        for (Long k: boardDTOMap.keySet()){
            System.out.println(boardDTOMap.get(k).getId()+"\t "+boardDTOMap.get(k).getBoardTitle()+"\t "
                    +boardDTOMap.get(k).getBoardWriter()+"\t "+boardDTOMap.get(k).getBoardHits()+"\t "+boardDTOMap.get(k).getCreatedAt());
        }
    }
    ...
    public void search() {
        ...
        Map<Long,BoardDTO>boardDTOMap = boardRepository.search(question);
        for (Long k: boardDTOMap.keySet()){
            System.out.println("boardDTO = " + boardDTOMap.get(k));
        }
    }
